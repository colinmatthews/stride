...
version: alpha
name: Stride
description: Editorial, athletic design system for an endurance training product. Bold ink + bone + signal-red palette; display serif-style geometric + utilitarian mono; spacious, data-dense, ticker-uppercase eyebrows.
colors:
  background: "#FAFAF7"
  foreground: "#2A241E"
  surface: "#FFFFFF"
...
Visual register:
- **Quiet bone background**, **deep ink** for gravitas, **signal red** reserved for the single thing that matters per screen (KOM, CTA, primary action).
...
Palette roles:
- **`primary` (signal red, `oklch(0.6 0.22 25)`)** — the one color that means "this is the thing." KOM/QOM indicators, the single primary CTA on a screen, active tab accents, progress fills, eyebrow accents on editorial pages. Never use for body text or as a background for more than ~10% of the screen.
...
- **Button** — variants `default` (primary red), `secondary` (ink), `outline`, `ghost`, `link`. One `default` button per screen max; use `outline` or `ghost` for secondary actions.
...
**Do:**
- Reserve signal red for the single most important thing per screen (CTA, KOM indicator, primary stat).
...
**Don't:**
- Don't use primary red as a field-level background (banners, full cards, hero panels). It overwhelms and loses its signal value.
