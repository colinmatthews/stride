...
Palette roles:
- **`primary` (signal blue, `oklch(0.55 0.24 263)`)** — the one color that means "this is the thing." KOM/QOM indicators, the single primary CTA on a screen, active tab accents, progress fills, eyebrow accents on editorial pages. Never use for body text or as a background for more than ~10% of the screen.
...
- **`kudos` / `pr`** — domain-specific semantic colors. `kudos` stays signal orange (the social-positive signal). `pr` is green — reserved for personal-record moments and "readiness: high" states.
