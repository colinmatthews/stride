...
export const Route = createFileRoute("/clubs")({
  head: () => ({ meta: [{ title: "Clubs — Stride" }, { name: "description", content: "Join clubs and train with people who share your goals." }] }),
  component: ClubsPage,
});
        ...
        {CLUBS.map((c) => (
          <article key={c.id} className="bg-surface border border-border rounded-xl overflow-hidden">
            ...
            <div className="p-4">
              ...
              <div className="text-xs text-muted-foreground mt-0.5">{c.city} · {c.sport}</div>
              ...
              <div className="mt-4 flex items-center justify-between">
                ...
                <button
                  ...
                  className={`text-xs px-3 py-1.5 rounded-md font-medium inline-flex items-center gap-1.5 ${
                    joined[c.id] ? "bg-secondary text-secondary-foreground" : "bg-primary text-primary-foreground"
                  }`}
                >
                  {joined[c.id] ? <><Check className="h-3 w-3" /> Joined</> : "Join"}
                </button>
              </div>
            </div>
          </article>
