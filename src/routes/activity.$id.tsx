...
import { Heart, MessageCircle, Trophy, Share2, Flag, MapPin, Mountain, Clock, Zap } from "lucide-react";
import { ACTIVITIES, fmtDate, fmtDuration, fmtPace, getActivity, getAthlete, getSegment, elevationProfile } from "@/lib/mock-data";
  ...
  head: ({ loaderData }) => ({
    meta: loaderData
      ? [
          ...
          { name: "description", content: `${loaderData.activity.sport} · ${loaderData.activity.distanceKm.toFixed(2)} km` },
        ]
      : [],
  }),
  ...
  notFoundComponent: () => (
    <AppShell>
      <div className="text-center py-24">
        ...
        <Link to="/" className="text-primary mt-2 inline-block">Back to feed</Link>
      </div>
    </AppShell>
  ),
});
...
function ActivityDetail() {
  ...
  const submitComment = () => {
    ...
    const c = { id: `nc-${Date.now()}`, athleteId: "me", text: comment.trim() };
    setComments((cs) => [...cs, c]);
    activity.comments.push(c);
    setComment("");
  };

  const toggleKudos = () => {
    setKudoed((k) => !k);
    setKudosCount((c) => c + (kudoed ? -1 : 1));
  };
      ...
      <div className="grid grid-cols-[1fr_360px] gap-8">
        <div className="min-w-0">
          ...
          <div className="flex items-start gap-4 mb-6">
            <Link to="/athlete/$id" params={{ id: ath.id }}>
              <img src={ath.avatar} alt={ath.name} className="h-12 w-12 rounded-full object-cover" />
            </Link>
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                <Link to="/athlete/$id" params={{ id: ath.id }} className="font-medium hover:underline">
                  {ath.name}
                </Link>
          ...
          <h1 className="text-4xl font-display font-bold tracking-tight text-balance">{activity.title}</h1>
          ...
          <div className="grid grid-cols-4 gap-6 my-8 pb-8 border-b border-border">
            ...
            {activity.sport === "Ride" ? (
              ...
              <Stat label="Pace" value={activity.avgPaceSecPerKm ? fmtPace(activity.avgPaceSecPerKm).replace("/km", "") : "—"} unit="/km" />
            )}
          ...
          <section className="rounded-xl overflow-hidden border border-border">
            <RouteMap seed={activity.routeSeed} width={1000} height={420} className="w-full h-[380px]" />
          </section>
          {activity.photo && (
            <img src={activity.photo} alt={activity.title} className="w-full h-[380px] object-cover rounded-xl border border-border mt-4" />
          )}
              ...
              <div className="h-52">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={elev}>
                    ...
                    <Area dataKey="elev" stroke="var(--primary)" strokeWidth={2} fill="url(#elevGrad)" />
                  </AreaChart>
                </ResponsiveContainer>
              </div>
            </div>
                    ...
                    <tbody>
                      {splits.map((s) => (
                        <tr key={s.km} className="border-t border-border">
                          ...
                          <td className="py-2 font-mono">{fmtPace(s.paceSec).replace("/km", "")}</td>
                          <td className="py-2 font-mono">{s.hr}</td>
                          <td className="py-2 font-mono text-right">{s.elev > 0 ? `+${s.elev}` : s.elev}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                    ...
                    <li key={eff.id}>
                      ...
                      >
                        <div>
                          ...
                          <div className="text-xs text-muted-foreground">{seg.distanceKm.toFixed(1)} km · {seg.avgGrade.toFixed(1)}%</div>
                        </div>
                        <div className="text-right">
                          <div className="stat-num text-base">#{eff.rank}</div>
                          <div className="text-[11px] text-muted-foreground uppercase tracking-wider">your rank</div>
                        </div>
                      </Link>
                    </li>
                  );
              ...
              <div className="bg-surface rounded-xl border border-border p-4 h-44">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={splits}>
                    <CartesianGrid stroke="var(--border)" vertical={false} />
                    <XAxis dataKey="km" tick={{ fontSize: 11, fill: "var(--muted-foreground)" }} axisLine={false} tickLine={false} />
                    <YAxis tick={{ fontSize: 11, fill: "var(--muted-foreground)" }} axisLine={false} tickLine={false} width={36} domain={["dataMin - 5", "dataMax + 5"]} />
                    <Tooltip contentStyle={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 8, fontSize: 12 }} />
          ...
          <section id="comments" className="mt-10 pt-8 border-t border-border">
            <div className="flex items-center gap-3">
              <button
                ...
                className={`inline-flex items-center gap-2 px-4 py-2 rounded-md border transition-colors ${
                  kudoed ? "bg-primary text-primary-foreground border-primary" : "border-border hover:bg-muted"
                }`}
              >
            ...
            <div className="mt-4 flex gap-3">
              <img src={getAthlete("me").avatar} alt="" className="h-8 w-8 rounded-full object-cover shrink-0" />
              <input
                value={comment}
              ...
              />
              <button onClick={submitComment} className="h-10 px-4 rounded-md bg-secondary text-secondary-foreground text-sm font-medium">
                Post
              </button>
            </div>
          </section>
        ...
        <aside className="space-y-5">
          <div className="bg-surface rounded-xl border border-border p-5">
            <h3 className="text-xs uppercase tracking-[0.14em] text-muted-foreground mb-3">Effort</h3>
            <div className="space-y-3">
              <Row label="Avg HR" value={activity.avgHr ? `${activity.avgHr} bpm` : "—"} />
              <Row label="Calories" value={`${Math.round(activity.movingSeconds / 60 * 10)} kcal`} />
              <Row label="Relative effort" value={`${Math.min(100, Math.round(activity.movingSeconds / 60 + activity.elevationM / 10))}`} />
            </div>
          </div>
          <div className="bg-surface rounded-xl border border-border p-5">
            <h3 className="text-xs uppercase tracking-[0.14em] text-muted-foreground mb-3">Other activities</h3>
            <ul className="space-y-3">
              {ACTIVITIES.filter((a) => a.id !== activity.id).slice(0, 4).map((a) => (
                <li key={a.id}>
                  <button
                  ...
                  >
                    <img src={getAthlete(a.athleteId).avatar} alt="" className="h-8 w-8 rounded-full object-cover" />
                    <div className="min-w-0 flex-1">
                      <div className="text-sm font-medium truncate">{a.title}</div>
                      <div className="text-xs text-muted-foreground">{a.distanceKm.toFixed(1)} km · {fmtDuration(a.movingSeconds)}</div>
                    </div>
                  </button>
                </li>
              ))}
