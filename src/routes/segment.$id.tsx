  ...
  head: ({ loaderData }) => ({
    meta: loaderData
      ? [
          ...
          {
            name: "description",
            content: `${loaderData.segment.distanceKm} km · ${loaderData.segment.avgGrade}% grade`,
          },
        ]
      : [],
  }),
...
function SegmentDetail() {
  ...
  return (
    <AppShell>
      <Link to="/segments" className="text-sm text-muted-foreground hover:text-foreground">
        ← All segments
      </Link>
      ...
      <div className="grid grid-cols-5 gap-6 my-8">
        <Stat
          label="Distance"
          value={units.distanceValue(segment.distanceKm, 1)}
        ...
        <Stat
          label="Elevation"
          value={units.elevationValue(segment.elevationM)}
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {leaderboard.map((row, i) => {
                  const paceSecPerKm = row.timeSec / segment.distanceKm;
                  ...
                  <li key={s.id}>
                    ...
                    >
                      ...
                      <div className="text-xs text-muted-foreground">
                        {units.distance(s.distanceKm, 1)} · {s.avgGrade.toFixed(1)}%
                      </div>
                    </Link>
                  </li>
                ))}
